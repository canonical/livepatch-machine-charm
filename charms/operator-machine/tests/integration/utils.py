...
async def build_snap_and_charm(root_directory: str, charm_directory: str, ops_test: OpsTest) -> Tuple[str, str]:
    """
...
async def build_snap_lxd(working_directory: Path) -> Path:
    ...
    for line in iter(p.stdout.readline, b''):
        ...
        # We check both incase the process does output b'' and we accidentally
...
async def check_if_snap_exists(git_dir: Path, snap_build_dir: Path) -> Tuple[bool, str]:
    ...
    git_version = check_output(_CMD, universal_newlines=True, cwd=git_dir)
    for fname in os.listdir(str(snap_build_dir)):
        if fname.endswith('.snap'):
            if git_version.strip() in fname.strip():
                path = os.path.abspath("{path}/{file}".format(path=str(snap_build_dir), file=fname.strip()))
                return True, path
            break
    return False, None
...
class FailedToBuildSnapError(Exception):
    ...
    def __init__(self, respcode):
        self.message = "Snap failed to build with response code: {respcode} ".format(respcode=respcode)
